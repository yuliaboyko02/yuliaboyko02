<!-- 💗 Banner -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:ec4899,100:f9a8d4&height=200&section=header&text=Yulia%20Boyko&fontSize=40&fontColor=ffffff"/>
</p>

## Hi there, I'm Yulia! 👋


🚀 **Data Analyst | Turning data into insights**

I’m passionate about analyzing data, building dashboards, and helping businesses make data-driven decisions.

---

## 🔧 Tools & Technologies

<p align="center">
  <img src="https://img.icons8.com/color/48/microsoft-excel-2019.png" width="100"/>
  <img src="https://img.icons8.com/color/48/tableau-software.png" width="100"/>
  <img src="https://img.icons8.com/color/48/power-bi.png" width="100"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" width="100"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="100"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/azure/azure-original.svg" width="100"/>
</p>

---


## 🛠️ Skills & Tools

### 📊 Excel

Advanced data analysis using formulas, pivot tables, dashboards, and data cleaning to extract meaningful insights.

---

### 📈 Tableau

Creating interactive and visually compelling dashboards for clear data storytelling and business insights.

---

### 📉 Power BI
 
Building dynamic reports and dashboards with DAX and data modeling to support decision-making.

---

### 🗄️ SQL
 
Writing optimized queries to extract, transform, and analyze structured data efficiently.

---
### ☁️ Azure

Working with Azure services for cloud-based data storage and analytics solutions.

---

### 🐍 Python

Using Pandas, NumPy, and Matplotlib for data cleaning, analysis, and visualization.

---

## 🌱 About Me

⭐ Analytical and detail-oriented  
⭐ Passionate about data storytelling  
⭐ Continuously learning and improving  

<!-- 💗 Footer -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:ec4899,100:f9a8d4&height=120&section=footer"/>
</p>
